...
parameters = FullEphemerisPropagator.NbodySRP_params(
    et0,
    lstar,
    mus,
    naif_ids;
    f_jacobian = FullEphemerisPropagator.symbolic_NbodySRP_jacobian(length(mus)),
    naif_frame=naif_frame,
    abcorr=abcorr
)

et_range = (et0, et0 + 365.0*86400)
interp_params = FullEphemerisPropagator.InterpolatedNbodySRP_params(et_range, parameters, 20000;
     rescale_epoch = false,)
...
function prob_func_NbodySRP(ode_problem, i, repeat)
    _x0 = x0_conditions[i]
    remake(ode_problem, u0=_x0)
end

prob = ODEProblem(FullEphemerisPropagator.eom_NbodySRP_SPICE!, u0, tspan, parameters)
...
function prob_func_NbodySRP_STM(ode_problem, i, repeat)
    _x0 = x0_conditions[i]
    remake(ode_problem, u0=_x0)
end

prob = ODEProblem(FullEphemerisPropagator.eom_NbodySRP_STM_SPICE!, u0, tspan, parameters)
ensemble_prob = EnsembleProblem(
    prob;
