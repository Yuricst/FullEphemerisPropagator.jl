...
struct InterpolatedEphemeris
    ...
    function InterpolatedEphemeris(
        ...
        splines = [
            Spline1D(times_input, rs[1,:]),
            Spline1D(times_input, rs[2,:]),
            Spline1D(times_input, rs[3,:]),
        ]
...
"""Interpolate ephemeris at a given epoch"""
    ...
    function InterpolatedNbodyParams(
        ...
        for i = 2:length(params.mus_scaled)
            ...
            ephem_dict[i] = InterpolatedEphemeris(
                params.naif_ids[2],
                ets,
                hcat([r_lt[1] for r_lt in rs_lt]...)/lstar_rescale,
                rescale_epoch,
                params.tstar
            )
        end
